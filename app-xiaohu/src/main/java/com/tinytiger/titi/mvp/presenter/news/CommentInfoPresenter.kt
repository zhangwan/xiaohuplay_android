 ...
 * @doc
 */
class CommentInfoPresenter : BasePresenter<CommentInfoContract.View>(),
    CommentInfoContract.Presenter {
...
//        mRootView?.showLoading()
        val disposable = mModel.indexComment(content_id, page, 0)
            .subscribe({ bean ->

                mRootView?.apply {

                    if (bean.code == 200) {
                        mRootView?.showComment(bean.data)
                    } else {
                        getErrorCode(bean)
                    }
                    dismissLoading()
