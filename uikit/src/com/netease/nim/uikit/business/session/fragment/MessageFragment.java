...
import com.netease.nim.uikit.business.session.module.input.ChatInputPanel;
...
public class MessageFragment extends TFragment implements ModuleProxy {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        parseIntent();
        View messageActivityBottomLayout = rootView.findViewById(R.id.messageActivityBottomLayout);
        if (sessionId.equals(getString(R.string.im_secretary))) {
    ...
    private void parseIntent() {
        ...
        if (inputPanel == null) {
            //  inputPanel = new InputPanel(container, rootView, getActionList(),1);

            inputPanel = new ChatInputPanel(container, rootView);
    ...
    public void setNoMsg(int follow, int yuer_letter1,int me_letter) {
       /* if (me_letter == 2) {
            if (follow<1){
                msgerr="您已开启私信限制，暂时不能发送消息。需关闭私信限制或关注对方才可发送";
                msgType = false;
            }
        }*/
    ...
    public boolean sendMessage(IMMessage message) {
        if (!msgType) {
            ToastUtils.show(getContext(), msgerr);
            return false;
        }
        if (!checkLocalAntiSpam(message)) {
            ToastUtils.show(getContext(), "你所发送的内容涉及敏感词汇，请重新输入");
            return false;
        }
