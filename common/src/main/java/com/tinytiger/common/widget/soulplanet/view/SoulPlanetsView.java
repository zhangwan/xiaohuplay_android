package com.tinytiger.common.widget.soulplanet.view;
...
import com.tinytiger.common.widget.soulplanet.PlanetCalculator;
import com.tinytiger.common.widget.soulplanet.adapter.NullPlanetAdapter;
...
public class SoulPlanetsView extends ViewGroup implements Runnable, PlanetAdapter.OnDataSetChangeListener {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            mode = typedArray.getInteger(R.styleable.SoulPlanetsView_autoScrollModeSoul, MODE_DISABLE);
            setManualScroll(typedArray.getBoolean(R.styleable.SoulPlanetsView_manualScrollSoul, true));
            mAngleX = typedArray.getFloat(R.styleable.SoulPlanetsView_startAngleXSoul, 0.5f);
            mAngleY = typedArray.getFloat(R.styleable.SoulPlanetsView_startAngleYSoul, 0.5f);
            setLightColor(typedArray.getColor(R.styleable.SoulPlanetsView_lightColorSoul, Color.WHITE));
            setDarkColor(typedArray.getColor(R.styleable.SoulPlanetsView_darkColorSoul, Color.BLACK));
            setRadiusPercent(typedArray.getFloat(R.styleable.SoulPlanetsView_radiusPercentSoul, radiusPercent));
            setScrollSpeed(typedArray.getFloat(R.styleable.SoulPlanetsView_scrollSpeedSoul, 2f));
            typedArray.recycle();
        }
    ...
    private void processTouch() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (child != null && child.getVisibility() != GONE) {
                ...
                // 设置透明度
                child.setAlpha(planetModel.getAlpha() + 0.05f);
    ...
    public void run() {
        ...
        if (!isOnTouch && mode != MODE_DISABLE) {
            ...
            if (mode == MODE_DECELERATE) {
                if (Math.abs(mAngleX) > 0.5f) {
                    mAngleX -= mAngleX * 0.1f;
                }
                if (Math.abs(mAngleY) > 0.5f) {
